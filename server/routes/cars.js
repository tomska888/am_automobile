    ...
    try {
      const [result] = await pool.query(
        `INSERT INTO cars
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          make, model, year, price, mileage || null, fuel_type,
          transmission || null, body_type || null, engine_size || null,
          power_hp || null, color || null, vin || null, status,
          featured ? 1 : 0, image_url || null,
  ...
  async (req, res) => {
    ...
    for (const field of allowed) {
      if (req.body[field] !== undefined) {
        updates.push(`${field} = ?`)
        values.push(field === 'featured' ? (req.body[field] ? 1 : 0) : req.body[field])
      }
    }
